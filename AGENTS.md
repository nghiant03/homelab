...
- `platforms/kuberay-operator/`, `platforms/kubescape-operator/` — Helm-based components: each is just `namespace.yaml` + `helm-repository.yaml` + `helm-release.yaml` with a pinned chart version. kubescape-operator values raise kubevuln's `ephemeral-storage` limit to 30Gi (grype DB + scan tarballs live in emptyDir and evicted the pod at the chart's 10Gi default).

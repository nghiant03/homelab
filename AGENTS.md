...
- The tailscale operator only supports `ts.net` machine names in ingress rules — a `*.home.arpa` rule host is ignored ("unsupported") and the proxy serves nothing; current operator versions also force HTTPS:443 on ingresses (no plain HTTP). To expose something as `*.home.arpa` over Tailscale, use a `type: LoadBalancer` + `loadBalancerClass: tailscale` Service (raw TCP passthrough, any port) with `tailscale.com/hostname` and `external-dns.kubernetes.io/hostname` annotations (see `platforms/gitea/service.yaml`). Note the prefix: external-dns v0.22+ uses `external-dns.kubernetes.io/`; the legacy `external-dns.alpha.kubernetes.io/` annotations are ignored.

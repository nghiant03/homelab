...
- `platforms/traefik/` — shared tailnet HTTP/HTTPS entry point. The `traefik-tailnet` Service in `kube-system` selects the existing k3s Traefik pods and is exposed by the Tailscale operator as `traefik-tailnet`. A `HelmChartConfig` customizes the existing k3s chart to publish this Service's load-balancer status to app Ingresses. No separate Traefik installation or node-IP overrides. See its README for staged rollout and verification.
